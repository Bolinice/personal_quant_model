...
async def get_events(
    ...
    events = EventService.get_events(
        db, stock_id=stock_id, event_type=event_type,
        start_date=start_date, end_date=end_date, severity=severity,
        limit=page_size, offset=offset,
    )
    return success_response(data=[{
        "id": e.id, "stock_id": e.stock_id, "event_type": e.event_type,
        "event_date": str(e.event_date), "severity": e.severity,
        "title": e.title, "score": float(e.score) if e.score else None,
    } for e in events])
...
async def get_event(event_id: int, db: Session = Depends(get_db)):
    ...
    return success_response(data={
        "id": event.id, "stock_id": event.stock_id, "event_type": event.event_type,
        "event_subtype": event.event_subtype, "event_date": str(event.event_date),
        ...
        "severity": event.severity, "score": float(event.score) if event.score else None,
        "title": event.title, "content": event.content, "source": event.source,
    })
...
async def get_risk_flags(
    ...
    return success_response(data=[{
        "trade_date": str(f.trade_date), "stock_id": f.stock_id,
        "blacklist_flag": f.blacklist_flag, "risk_penalty_score": float(f.risk_penalty_score) if f.risk_penalty_score else 0,
    } for f in flags])
