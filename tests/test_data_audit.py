...
class TestUniverseDataAudit:
    ...
    def test_stock_basic_exists_and_populated(self, db_conn):
        ...
        count = result.scalar()
        if count == 0:
            pytest.skip("stock_basic表为空, 跳过数据审计")
    ...
    def test_stock_daily_exists_and_populated(self, db_conn):
        ...
        count = result.scalar()
        if count == 0:
            pytest.skip("stock_daily表为空, 跳过数据审计")
    ...
    def test_stock_daily_basic_total_mv_coverage(self, db_conn):
        ...
        total = result.scalar()
        if total == 0:
            pytest.skip("stock_daily_basic表为空")
    ...
    def test_stock_daily_basic_date_format_consistency(self, db_conn):
        ...
        # 两者类型应一致
        assert isinstance(sd_date, type(sdb_date)), (
...
class TestAlphaFactorDataAudit:
    ...
    def test_stock_financial_exists_and_populated(self, db_conn):
        ...
        count = result.scalar()
        if count == 0:
            pytest.skip("stock_financial表为空, 跳过基本面因子审计")
    ...
    def test_stock_financial_roe_coverage(self, db_conn):
        ...
        total = result.scalar()
        if total == 0:
            pytest.skip("stock_financial表为空")
    ...
    def test_stock_financial_revenue_growth_coverage(self, db_conn):
        ...
        if total == 0:
            pytest.skip("stock_financial表为空")
        result = db_conn.execute(text("SELECT COUNT(*) FROM stock_financial WHERE revenue_yoy IS NOT NULL"))
        with_yoy = result.scalar()
        coverage = with_yoy / total if total > 0 else 0
        if coverage < 0.30:
            pytest.skip(f"revenue_yoy覆盖率{coverage:.1%}过低 — revenue_growth_yoy因子需从原始财务数据计算")
...
class TestDateFormatConsistency:
    ...
    stock_daily_basic.trade_date: date类型 (YYYY-MM-DD)
    stock_basic.list_date: integer类型 (YYYYMMDD)
    stock_financial.end_date/ann_date: character varying类型 (YYYYMMDD)
    """
    ...
    def test_date_format_cross_table_join(self, db_conn):
        """跨表JOIN时日期格式是否兼容"""
        # stock_daily.trade_date and stock_daily_basic.trade_date are both Date type
        result = db_conn.execute(
            text("""
        ...
        assert direct_count > 0, "stock_daily.trade_date 与 stock_daily_basic.trade_date JOIN失败! 数据无法关联"
...
class TestDataVolumeAudit:
    ...
    def test_active_stock_count(self, db_conn):
        ...
        count = result.scalar()
        if count == 0:
            pytest.skip("stock_basic无上市状态数据")
    ...
    def test_daily_coverage_per_trading_day(self, db_conn):
        ...
        for row in rows:
            assert row[1] >= 500, f"{row[0]}仅{row[1]}只股票, 不足500 — 数据覆盖不足"
    ...
    def test_stock_daily_basic_coverage_ratio(self, db_conn):
        ...
        if sd_stocks > 0:
            ...
            assert ratio >= 0.80, f"stock_daily_basic覆盖率{ratio:.1%}, 低于80% — 市值/PE/PB数据不完整"
...
class TestDataQualityAudit:
    ...
    def test_stock_financial_roe_reasonable_range(self, db_conn):
        ...
        if total > 0:
            ...
            assert outlier_ratio < 0.05, f"ROE异常值比例{outlier_ratio:.2%} (>5%) — 数据质量问题或需去极值"
    ...
    def test_stock_daily_vol_amount_consistency(self, db_conn):
        ...
        if total_suspended > 0:
            ...
            assert ratio < 0.10, f"vol=0但amount>0的比例{ratio:.2%} (>10%) — 数据不一致"
...
class TestMissingDataTables:
    ...
    def test_stock_margin_available(self, db_conn):
        ...
        if count == 0:
            pytest.skip(
                "stock_margin为空 — margin_balance_change因子无法计算. 建议: 通过Tushare margin接口补充融资融券数据"
            )
...
class TestFactorComputationFeasibility:
    ...
    def test_flow_confirm_factors_from_db(self, db_conn):
        ...
        if north_count == 0 and flow_count == 0:
            pytest.skip("stock_northbound和stock_money_flow均为空 — FlowConfirm模块将退化, 至少需要其中一个数据源")
    ...
    def test_universe_build_from_real_data(self, db_conn):
        ...
        # 从数据库读取数据
        stock_basic_df = pd.read_sql("SELECT ts_code, list_date, list_status FROM stock_basic LIMIT 200", db_conn)
        ...
        price_df = pd.read_sql(
            text("SELECT ts_code, trade_date, close, amount FROM stock_daily WHERE trade_date >= :cutoff"),
            db_conn,
        ...
        assert len(universe) > 0, "使用真实数据构建的股票池为空 — 数据可能存在严重问题"
