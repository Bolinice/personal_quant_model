...
def read_alert_log(log_id: int, db: Session = Depends(SessionLocal)):
...
def create_alert_log_endpoint(log: AlertLogCreate, db: Session = Depends(SessionLocal)):
    return create_alert_log(log, db=db)

@router.put("/{log_id}", response_model=AlertLogOut)
def update_alert_log_endpoint(log_id: int, log_update: AlertLogUpdate, db: Session = Depends(SessionLocal)):
...
def delete_alert_log_endpoint(log_id: int, db: Session = Depends(SessionLocal)):
...
def trigger_risk_alerts(portfolio_id: int, date: str, db: Session = Depends(SessionLocal)):
...
def trigger_performance_alerts(portfolio_id: int, date: str, db: Session = Depends(SessionLocal)):
...
def trigger_all_alerts(portfolio_id: int, date: str, db: Session = Depends(SessionLocal)):
