...
import type {
  PricingOverview,
  SubscriptionPlan,
  PricingMatrix as PricingMatrixType,
  UpgradePackage as UpgradePackageType,
...
export default function PricingPage() {
  ...
  const contentMatrixData =
    billingCycle === 'yearly'
      ? s('pricing_matrix')?.extra?.yearly
      : s('pricing_matrix')?.extra?.monthly;
  const matrixPools = apiMatrixForCycle ? apiMatrixForCycle.pools : contentMatrixData?.pools || [];
  const matrixFrequencies = apiMatrixForCycle
    ? apiMatrixForCycle.frequencies
    : contentMatrixData?.frequencies || [];
  const matrixPrices = apiMatrixForCycle
    ? apiMatrixForCycle.prices
    : contentMatrixData?.prices || [];
    ...
    <Box>
        ...
        <Container maxWidth="sm" sx={{ position: 'relative', textAlign: 'center' }}>
          <motion.div {...fadeUp}>
            <Typography variant="h3" sx={{ fontWeight: 800, mb: 2 }}>
              {s('pricing_header')?.title || '定价方案'}
            </Typography>
            <Typography sx={{ color: '#94a3b8', mb: 2, lineHeight: 1.8 }}>
              {s('pricing_header')?.subtitle}
            </Typography>
            <Typography sx={{ color: '#94a3b8', fontSize: '0.9rem', lineHeight: 1.8 }}>
              {s('pricing_header')?.body}
            </Typography>
          </motion.div>
        </Container>
      </Box>
          ...
          <Container maxWidth="md">
            <motion.div {...fadeUp}>
              <Typography variant="h5" sx={{ fontWeight: 700, mb: 2, textAlign: 'center' }}>
                {s('pricing_philosophy')?.title}
              </Typography>
              <Typography sx={{ color: '#94a3b8', textAlign: 'center', mb: 4 }}>
                {s('pricing_philosophy')?.body}
              </Typography>
            </motion.div>
            <Box
              sx={{
                  ...
                  >
                    ...
                    <Box sx={{ mb: 2 }}>
                      ...
                      {plan.price_yearly && (
                        <Typography sx={{ fontWeight: 800, fontSize: '1.5rem' }}>
                          ...
                          <Typography
                            component="span"
                            sx={{ color: '#94a3b8', fontSize: '0.8rem', fontWeight: 400 }}
                          >
                            /年
                          </Typography>
                        </Typography>
                    ...
                    <Box sx={{ flex: 1, mb: 2 }}>
                      {(plan.features || []).map((feat: string) => (
                        <Box
                          key={feat}
                          sx={{ display: 'flex', alignItems: 'flex-start', gap: 0.75, mb: 0.5 }}
                        >
                          <CheckCircleIcon
                            sx={{ color: '#22d3ee', fontSize: 14, mt: 0.3, flexShrink: 0 }}
                          />
                          <Typography
                            sx={{ color: '#cbd5e1', fontSize: '0.8rem', lineHeight: 1.5 }}
                          >
                            {feat}
                          </Typography>
                        </Box>
                    ...
                    <Box sx={{ mt: 'auto' }}>
                      <Button
                        ...
                        size="small"
                        onClick={() =>
                          isApiPlan ? navigate('/app/subscribe') : navigate('/pricing')
                        }
                        sx={
                          plan.highlight
                            ? {}
                            : { borderColor: 'rgba(148,163,184,0.25)', color: '#e2e8f0' }
                        }
                      >
                        {plan.buttons?.[0] || '选择方案'}
                      </Button>
          ...
          <Container maxWidth="lg">
            <motion.div {...fadeUp}>
              <Typography variant="h4" sx={{ fontWeight: 700, mb: 4, textAlign: 'center' }}>
                {s('pricing_comparison')?.title}
              </Typography>
            </motion.div>
            <motion.div {...fadeUp}>
              <TableContainer
                component={Paper}
                sx={{ background: 'rgba(15,23,42,0.6)', border: '1px solid rgba(148,163,184,0.1)' }}
              >
                <Table size="small">
                  <TableHead>
                    <TableRow>
                  ...
                  <TableBody>
                    {(s('pricing_comparison')?.extra?.rows || []).map(
                      (row: string[], i: number) => (
                        <TableRow key={i}>
                          {row.map((cell, j) => (
                            <TableCell
          ...
          <Container maxWidth="md">
            <motion.div {...fadeUp}>
              <Typography variant="h4" sx={{ fontWeight: 700, mb: 2, textAlign: 'center' }}>
                {s('pricing_matrix')?.title || '单模型订阅价格'}
              </Typography>
              <Typography sx={{ color: '#94a3b8', textAlign: 'center', mb: 3 }}>
                {s('pricing_matrix')?.subtitle}
              </Typography>
            </motion.div>
            ...
            {(apiMatrixForCycle?.note || s('pricing_matrix')?.extra?.note) && (
              <Typography sx={{ color: '#64748b', fontSize: '0.8rem', mt: 2, textAlign: 'center' }}>
                {apiMatrixForCycle?.note || s('pricing_matrix')?.extra?.note}
              </Typography>
            )}
          </Container>
      ...
      {upgradePkgs.length > 0 && (
        <Box sx={{ py: 10, backgroundColor: 'rgba(15,23,42,0.3)' }}>
          <Container maxWidth="lg">
            <motion.div {...fadeUp}>
              <Typography variant="h4" sx={{ fontWeight: 700, mb: 6, textAlign: 'center' }}>
                {s('upgrade_packages')?.title || '可选升级项'}
              </Typography>
            </motion.div>
            <Box
              sx={{
              ...
              {upgradePkgs.map((pkg: any, i: number) => (
                <motion.div key={i} {...fadeUp} transition={{ duration: 0.5, delay: i * 0.1 }}>
                  ...
                  >
                    ...
                    <Typography
                      sx={{ color: '#94a3b8', fontSize: '0.85rem', mb: 2, lineHeight: 1.7 }}
                    >
                      {pkg.description || pkg.desc}
                    </Typography>
                    <Box sx={{ mt: 'auto' }}>
                      {pkg.price_monthly && (
                        <Typography sx={{ color: '#22d3ee', fontWeight: 600, fontSize: '0.9rem' }}>
                          ¥{pkg.price_monthly}/月
                        </Typography>
                      )}
                      {pkg.price_yearly && (
                      ...
                      {pkg.price_standard && (
                        <Typography sx={{ color: '#22d3ee', fontWeight: 600, fontSize: '0.9rem' }}>
                          标准: {pkg.price_standard}
                        </Typography>
                      )}
                      {pkg.price_advanced && (
                        <Typography sx={{ color: '#22d3ee', fontWeight: 600, fontSize: '0.9rem' }}>
                          增强: {pkg.price_advanced}
                        </Typography>
                      )}
                      {pkg.price_unit && (
                        <Typography sx={{ color: '#22d3ee', fontWeight: 600, fontSize: '0.9rem' }}>
              ...
              {(s('combo_discounts')?.extra?.rules || []).map((rule: any) => (
                ...
                >
                  ...
                  <Chip
                    label={rule.discount}
                    size="small"
                    sx={{ backgroundColor: 'rgba(34,211,238,0.1)', color: '#22d3ee' }}
                  />
                </Box>
              ))}
              {s('combo_discounts')?.extra?.same_pool_rules && (
                <>
                  <Typography sx={{ color: '#64748b', fontSize: '0.8rem', mt: 2, mb: 1 }}>
                    {t.pricing.samePool}
                  </Typography>
              ...
              {(s('audience_selector')?.extra?.options || []).map((opt: any, i: number) => (
                <motion.div key={i} {...fadeUp} transition={{ duration: 0.5, delay: i * 0.1 }}>
                  ...
                  >
                    ...
                    <Typography sx={{ fontWeight: 700, color: '#22d3ee', mb: 1 }}>
                      {opt.recommendation}
                    </Typography>
      ...
      {s('risk_disclaimer') && (
        <Box sx={{ py: 6, backgroundColor: 'rgba(15,23,42,0.3)' }}>
          <Container maxWidth="sm">
            <Typography sx={{ fontWeight: 600, fontSize: '0.9rem', mb: 1, textAlign: 'center' }}>
              {s('risk_disclaimer')?.title}
            </Typography>
            <Typography
              sx={{ color: '#64748b', fontSize: '0.8rem', lineHeight: 1.8, textAlign: 'center' }}
            >
          ...
          <Container maxWidth="sm" sx={{ position: 'relative', textAlign: 'center' }}>
            <motion.div {...fadeUp}>
              ...
              <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center', flexWrap: 'wrap' }}>
                <Button variant="contained" size="large" onClick={() => navigate('/app/subscribe')}>
                  {t.btn.trial}
                </Button>
