...
class TestUniverseBuilder:
    ...
    def test_build_basic(self):
        ...
        result = builder.build(date(2024, 1, 15), stock_basic_df, price_df,
                               min_list_days=0, min_daily_amount=0, min_price=0)
        assert len(result) >= 1
    ...
    def test_price_filter(self):
        ...
        result = builder.build(date(2024, 1, 15), stock_basic_df, price_df,
                               min_list_days=0, min_daily_amount=0, min_price=3.0)
        assert '000001.SZ' in result
        assert '000002.SZ' not in result
    ...
    def test_filter_risk_events(self):
        ...
        filtered, reasons = builder.filter_risk_events(
            candidates, risk_events_df, date(2024, 1, 15)
        )
    ...
    def test_filter_blacklist(self):
        ...
        filtered, reasons = builder.filter_blacklist(
            candidates, blacklist_df, date(2024, 1, 15)
        )
        assert '000002.SZ' not in filtered
        assert '000001.SZ' in filtered
...
class TestRegimeDetector:
    ...
    def test_weight_adjustments(self):
        ...
        base_weights = {'quality_growth': 0.35, 'expectation': 0.30,
                        'residual_momentum': 0.25, 'flow_confirm': 0.10}
...
class TestFactorMonitor:
    ...
    def test_module_correlation_matrix(self):
        ...
        np.random.seed(42)
        module_scores = pd.DataFrame(np.random.randn(100, 4),
                                     columns=['quality', 'expectation', 'momentum', 'flow'])
...
class TestCompliance:
    ...
    def test_compliance_module_exists(self):
        from app.core.compliance import check_high_risk_text, add_disclaimer
