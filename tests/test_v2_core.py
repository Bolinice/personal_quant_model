...
class TestModuleRegistry:
    ...
    def test_get_risk_penalty_module(self):
        from app.core.alpha_modules import get_risk_penalty_module, RiskPenaltyModule
...
class TestEnsembleEngine:
    ...
    def test_fuse_with_precomputed_module_scores(self):
        ...
        # 不使用预计算结果
        scores2, meta2 = engine.fuse(df, regime='trending')
    ...
    def test_fuse_without_risk_penalty(self):
        ...
        scores, meta = engine.fuse(df, regime='trending', apply_risk_penalty=False)
    ...
    def test_step5_normalize_bounds(self):
        from app.core.ensemble import EnsembleEngine, MIN_WEIGHT, MAX_WEIGHT
        engine = EnsembleEngine()
    ...
    def test_regime_weight_consistency(self):
        ...
        from app.core.ensemble import REGIME_WEIGHT_ADJUSTMENTS as ensemble_adj
        from app.core.regime import REGIME_WEIGHT_ADJUSTMENTS as regime_adj

        for regime in ['risk_on', 'trending', 'defensive', 'mean_reverting']:
            assert regime in ensemble_adj
            assert regime in regime_adj
            for module in ensemble_adj[regime]:
                assert ensemble_adj[regime][module] == regime_adj[regime][module], \
...
class TestUniverseBuilder:
    ...
    def test_build_min_price_filter(self):
        ...
        result = builder.build(
            ...
            self._make_stock_basic(),
            price_df,
            min_list_days=0, min_daily_amount=0, min_price=3.0, min_market_cap=0,
        )
        assert '000001.SZ' not in result
    ...
    def test_filter_risk_events(self):
        ...
        filtered, reasons = builder.filter_risk_events(candidates, risk_events, trade_date)
    ...
    def test_filter_blacklist(self):
        ...
        filtered, reasons = builder.filter_blacklist(candidates, blacklist, date(2024, 1, 15))
    ...
    def test_filter_limit_up_down_vectorized(self):
        ...
        filtered, reasons = builder.filter_limit_up_down(candidates, price_df, trade_date)
...
class TestPortfolioBuilderV2:
    ...
    def test_rebalance_buffer(self):
        ...
        from app.core.portfolio_builder import PortfolioBuilder, PortfolioMode, REBALANCE_BUFFER
        builder = PortfolioBuilder(mode=PortfolioMode.PRODUCTION)
        scores = self._make_scores()
        ...
        result = builder.build_production_portfolio(
            scores, current_holdings=current_holdings
        )
        assert not result.empty
    ...
    def test_extreme_risk_zero_weight(self):
        """极高风险(extreme)权重应为0"""
        from app.core.portfolio_builder import PortfolioBuilder, PortfolioMode, RISK_DISCOUNT
...
class TestRiskBudgetEngine:
    ...
    def test_optimize_with_linear_constraints(self):
        ...
        weights = engine.optimize_with_risk_constraints(
            alpha, exposures, factor_cov, risk_budget
        )
...
class TestIntegration:
    ...
    def test_regime_ensemble_consistency(self):
        ...
        from app.core.regime import RegimeDetector
        from app.core.ensemble import EnsembleEngine, REGIME_WEIGHT_ADJUSTMENTS
    ...
    def test_universe_to_portfolio_flow(self):
        ...
        universe = builder_u.build(
            trade_date, stock_basic, price_df,
            min_list_days=0, min_daily_amount=0, min_price=0, min_market_cap=0,
        )
