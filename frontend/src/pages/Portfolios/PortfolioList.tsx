...
export default function PortfolioList() {
  ...
  const loadPortfolios = async () => {
    ...
    } catch { setSnackbar({ open: true, message: '加载组合失败', severity: 'error' }); }
    finally { setLoading(false); }
  };

  useEffect(() => { if (selectedModel) loadPortfolios(); }, [selectedModel]);
    ...
    <Box>
      ...
      <Dialog open={genDialog} onClose={() => setGenDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleGenerate}>生成</Button>
        </DialogActions>
      </Dialog>
