...
class TestBaseDataSource:
    ...
    def test_retry_call_exhausted(self):
        ...
        with patch("time.sleep"), pytest.raises(ValueError, match="permanent error"):
...
class TestDataSourceManager:
    ...
    def test_register_and_get(self):
        from app.data_sources.base import BaseDataSource, DataSourceManager
        ...
        class MockSource(BaseDataSource):
            ...
            def get_stock_daily(self, **kw):
                return pd.DataFrame()

            def get_index_daily(self, **kw):
                return pd.DataFrame()

            def get_financial_data(self, **kw):
                return pd.DataFrame()

            def get_trading_calendar(self, **kw):
                return pd.DataFrame()
...
class TestAKShareSource:
    ...
    def test_fetch_price(self):
        ...
        source = self._make_source()
        with patch.object(
            source, "_fetch_stock_daily", return_value=pd.DataFrame({"trade_date": ["20240102"]})
        ) as mock:
            source.fetch("price", ts_code="600000.SH")
    ...
    def test_fetch_basic(self):
        ...
        with patch.object(source, "_fetch_stock_daily_basic", return_value=pd.DataFrame()) as mock:
            source.fetch("basic", trade_date=date(2024, 1, 2))
            mock.assert_called_once()
    ...
    def test_fetch_financial(self):
        ...
        with patch.object(source, "_fetch_financial", return_value=pd.DataFrame()) as mock:
            source.fetch("financial", ts_code="600000.SH")
            mock.assert_called_once()
    ...
    def test_fetch_index(self):
        ...
        with patch.object(source, "_fetch_index_daily", return_value=pd.DataFrame()) as mock:
            source.fetch("index", ts_code="000001.SH")
            mock.assert_called_once()
    ...
    def test_fetch_moneyflow(self):
        ...
        with patch.object(source, "_fetch_moneyflow", return_value=pd.DataFrame()) as mock:
            source.fetch("moneyflow", trade_date=date(2024, 1, 2))
            mock.assert_called_once()
    ...
    def test_fetch_margin(self):
        ...
        with patch.object(source, "_fetch_margin", return_value=pd.DataFrame()) as mock:
            source.fetch("margin", trade_date=date(2024, 1, 2))
            mock.assert_called_once()
...
class TestCrawlerDataSource:
    ...
    def test_parse_sina_quote(self):
        ...
        # 新浪行情格式：至少32个字段
        fields = ",".join(
            ["浦发银行", "10.00", "9.90", "10.50", "11.00", "9.80", "10.40", "10.50", "50000", "5500000.00"]
            + ["0"] * 22
        )  # padding to 32 fields
...
class TestDataCleaner:
    ...
    def test_clean_stock_daily_normal(self):
        ...
        _result, report = cleaner.clean_stock_daily(df)
    ...
    def test_clean_stock_daily_limit_violation_gem(self):
        ...
        result, _report = cleaner.clean_stock_daily(df)
    ...
    def test_clean_index_daily(self):
        ...
        _result, report = cleaner.clean_index_daily(df)
    ...
    def test_clean_stock_basic(self):
        ...
        result, _report = cleaner.clean_stock_basic(df)
        # 北交所股票应该被过滤
