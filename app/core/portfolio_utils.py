...
@with_db
def get_portfolio_positions(portfolio_id: int, trade_date: Optional[str] = None, db: Session = None) -> List[PortfolioPosition]:
    """获取组合持仓"""
...
def update_portfolio_nav(portfolio_id: int, nav: float, trade_date: str, db: Session = None) -> SimulatedPortfolioNav:
    """更新组合净值"""
    nav_record = SimulatedPortfolioNav(
...
@with_db
def get_portfolio_nav_history(portfolio_id: int, start_date: str, end_date: str, db: Session = None) -> List[SimulatedPortfolioNav]:
    """获取组合净值历史"""
    return db.query(SimulatedPortfolioNav).filter(
