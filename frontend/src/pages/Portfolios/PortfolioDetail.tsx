...
export default function PortfolioDetail() {
  ...
  useEffect(() => {
    if (id) {
      ...
      portfolioApi
        .getRebalances(Number(id), startDate, endDate)
        .then((res) => setRebalances(res.data))
        .catch((e) => {
  ...
  const rebalanceNeonColor: Record<string, 'cyan' | 'purple' | 'amber'> = {
    scheduled: 'cyan',
    signal: 'purple',
    risk: 'amber',
  };
  const rebalanceTypeLabel: Record<string, string> = {
    scheduled: '定期',
    signal: '信号',
    risk: '风控',
  };
            ...
            <TableBody>
              {rebalances.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell>
                    <NeonChip
                      label={rebalanceTypeLabel[r.rebalance_type] || r.rebalance_type}
                      size="small"
                      neonColor={rebalanceNeonColor[r.rebalance_type] || 'default'}
                    />
                  </TableCell>
