...
import {
  ...
  TableHead, TableRow, Button, Tabs, Tab, Snackbar, Alert,
} from '@mui/material';
...
const healthNeon: Record<string, 'green' | 'amber' | 'red' | 'default'> = {
  healthy: 'green', warning: 'amber', unhealthy: 'red',
};
...
export default function MonitorDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      monitorApi.getModelHealth().then((res) => setModelHealth(res.data)).catch(() => {}),
      monitorApi.getAlerts({ page_size: 50 }).then((res) => setAlerts(res.data)).catch(() => {}),
  ...
  const handleResolve = async (alertId: number) => {
    try {
      ...
      setAlerts((prev) => prev.map((a) => a.alert_id === alertId ? { ...a, resolved_flag: true } : a));
      ...
      <Grid container spacing={2.5} sx={{ mb: 3 }}>
        <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          <GlassPanel animate={false}>
            <Typography variant="body2" sx={{ color: '#64748b', mb: 1 }}>市场状态</Typography>
            {regime ? (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                <Box sx={{ width: 12, height: 12, borderRadius: '50%', bgcolor: regimeColor[regime.regime] || '#94a3b8' }} />
                <Typography sx={{ fontWeight: 700, color: regimeColor[regime.regime] || '#e2e8f0', fontSize: '1.2rem' }}>
        ...
        <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          <MetricCard label="未解决告警" value={unresolvedAlerts.length} color={unresolvedAlerts.length > 0 ? '#f43f5e' : '#10b981'} />
        </Grid>
      </Grid>
              ...
              <TableBody>
                {factorHealth.map((h, i) => (
                  <TableRow key={i} hover>
                    ...
                    <TableCell>
                      <NeonChip
                        label={h.health_status === 'healthy' ? '健康' : h.health_status === 'warning' ? '警告' : '异常'}
                        size="small"
                ...
                {factorHealth.length === 0 && <TableRow><TableCell colSpan={7} align="center">暂无因子健康数据</TableCell></TableRow>}
              </TableBody>
            </Table>
          </GlassTable>
        </GlassPanel>
              ...
              <TableBody>
                {modelHealth.map((h, i) => (
                  <TableRow key={i} hover>
                    ...
                    <TableCell>{h.prediction_drift != null ? h.prediction_drift.toFixed(4) : '-'}</TableCell>
                    <TableCell>{h.feature_importance_drift != null ? h.feature_importance_drift.toFixed(4) : '-'}</TableCell>
                ...
                {modelHealth.length === 0 && <TableRow><TableCell colSpan={6} align="center">暂无模型健康数据</TableCell></TableRow>}
              </TableBody>
            </Table>
          </GlassTable>
        </GlassPanel>
              ...
              <TableBody>
                {alerts.map((a) => (
                  <TableRow key={a.alert_id} hover>
                    ...
                    <TableCell sx={{ maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.message || '-'}</TableCell>
                    <TableCell>
                      <NeonChip label={a.resolved_flag ? '已解决' : '未解决'} size="small" neonColor={a.resolved_flag ? 'green' : 'red'} />
                    </TableCell>
                    <TableCell>
                      {!a.resolved_flag && (
                        <Button size="small" variant="outlined" onClick={() => handleResolve(a.alert_id)}>解决</Button>
                      )}
                    </TableCell>
                  </TableRow>
                ))}
                {alerts.length === 0 && <TableRow><TableCell colSpan={7} align="center">暂无告警</TableCell></TableRow>}
              </TableBody>
            </Table>
          </GlassTable>
        </GlassPanel>
