...
class FactorMonitor:
    ...
    def ic_drift(self, ic_series: pd.Series,
                  window: int = 60,
                  min_periods: int = 20) -> Dict[str, float]:
        """
        IC漂移检测 (GPT设计16.6节)
    ...
    def psi(self, current_dist: pd.Series,
    ...
    def rolling_ks(self, factor_values_t: pd.Series,
                    factor_values_ref: pd.Series) -> Dict[str, float]:
        """
        滚动KS检验 (GPT设计16.6节)
    ...
    def module_correlation_matrix(self, module_scores: pd.DataFrame,
    ...
    def check_health(self, ic_series: Optional[pd.Series] = None,
                      factor_values_current: Optional[pd.Series] = None,
                      factor_values_reference: Optional[pd.Series] = None,
                      module_scores: Optional[pd.DataFrame] = None,
        ...
        if ic_series is not None and not ic_series.empty:
            ...
            if len(ic_clean) > 20:
                ...
                if drift_result.get('is_decaying', False):
                    alerts.append(f"IC衰减: 近期IC={drift_result['ic_recent']:.4f}, 长期IC={drift_result['ic_long']:.4f}")
    ...
    def check_all_modules(self, module_ic_map: Dict[str, pd.Series],
                           module_scores: Optional[pd.DataFrame] = None) -> Dict[str, Dict]:
        """
        批量检查所有模块健康状态
