...
def run_daily_factor_calc(self, trade_date: str = None):
    ...
    try:
        ...
        from app.models.market import (
            StockFinancial, StockNorthbound, StockMoneyFlow, StockMargin,
            StockDailyBasic, StockShareholderPledge, StockTop10Holders,
            StockInstitutionalHolding, StockAnalystConsensus, StockBasic,
            StockStatusDaily, IndustryDaily,
        )
        ...
        try:
            fin_data = db.query(StockFinancial).filter(
                StockFinancial.ts_code.in_(ts_codes)
            ).all()
            if fin_data:
                financial_df = pd.DataFrame([{
                    'ts_code': f.ts_code, 'ann_date': f.ann_date, 'end_date': f.end_date,
                    'net_profit': f.net_profit, 'total_equity': f.total_equity,
                    'total_assets': f.total_assets, 'total_equity_prev': f.total_equity_prev,
                    'total_assets_prev': f.total_assets_prev,
                    'operating_cash_flow': f.operating_cash_flow,
                    'operating_revenue': f.operating_revenue, 'revenue': f.operating_revenue,
                    'gross_profit': f.gross_profit, 'current_assets': f.current_assets,
                    'current_liabilities': f.current_liabilities, 'goodwill': f.goodwill,
                    'total_market_cap': f.total_market_cap,
                    'pe_ttm': f.pe_ttm, 'pb': f.pb, 'ps_ttm': f.ps_ttm,
                    'dividend_yield': f.dividend_yield,
                    'revenue_yoy': f.revenue_yoy, 'net_profit_yoy': f.net_profit_yoy,
        ...
        try:
            mf_data = db.query(StockMoneyFlow).filter(
                StockMoneyFlow.ts_code.in_(ts_codes),
                StockMoneyFlow.trade_date == calc_date.strftime('%Y%m%d'),
            ).all()
            if mf_data:
                money_flow_df = pd.DataFrame([{
                    'ts_code': m.ts_code, 'smart_net_pct': m.smart_net_pct,
        ...
        try:
            mg_data = db.query(StockMargin).filter(
                StockMargin.ts_code.in_(ts_codes),
                StockMargin.trade_date == calc_date.strftime('%Y%m%d'),
            ).all()
            if mg_data:
                margin_df = pd.DataFrame([{
                    'ts_code': m.ts_code, 'margin_balance': m.margin_balance,
                } for m in mg_data])
        except Exception as e:
        ...
        try:
            ac_data = db.query(StockAnalystConsensus).filter(
                StockAnalystConsensus.ts_code.in_(ts_codes),
            ).all()
            if ac_data:
                consensus_df = pd.DataFrame([{
                    'ts_code': a.ts_code, 'effective_date': a.effective_date,
        ...
        try:
            sb_data = db.query(StockBasic).filter(
                StockBasic.ts_code.in_(ts_codes)
            ).all()
            if sb_data:
                stock_basic_df = pd.DataFrame([{
                    'ts_code': s.ts_code, 'list_date': s.list_date,
                } for s in sb_data])
        except Exception as e:
        ...
        try:
            id_data = db.query(IndustryDaily).filter(
                IndustryDaily.trade_date == calc_date
            ).all()
            if id_data:
                industry_df = pd.DataFrame([{
