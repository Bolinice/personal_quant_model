...
export default function PerformanceReportPage() {
  ...
  const industryData = report?.industry_exposure
    ? Object.entries(report.industry_exposure).map(([name, value]) => ({
        name,
        value: value * 100,
      }))
    : [];
    ...
    <Box>
      ...
      <GlassPanel sx={{ mb: 3 }}>
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <Button variant="contained" startIcon={<AssessmentIcon />} onClick={handleGenerate}>
            生成报告
          </Button>
          <Button variant="outlined" onClick={handleLoadAnalysis}>
            加载分析
          </Button>
        </Box>
      </GlassPanel>
        ...
        <Box sx={{ mb: 3 }}>
          ...
          <Grid container spacing={2.5}>
            <Grid size={{ xs: 6, sm: 4, md: 3 }}>
              <MetricCard
                label="总收益"
                value={`${(analysis.total_return * 100).toFixed(2)}%`}
                color={analysis.total_return >= 0 ? '#10b981' : '#f43f5e'}
              />
            </Grid>
            <Grid size={{ xs: 6, sm: 4, md: 3 }}>
              <MetricCard
            ...
            <Grid size={{ xs: 6, sm: 4, md: 3 }}>
              <MetricCard
                label="基准收益"
                value={`${(analysis.benchmark_return * 100).toFixed(2)}%`}
                color="#22d3ee"
              />
            </Grid>
            <Grid size={{ xs: 6, sm: 4, md: 3 }}>
            ...
            <Grid size={{ xs: 6, sm: 4, md: 3 }}>
              <MetricCard
                label="波动率"
                value={
                  analysis.volatility != null ? `${(analysis.volatility * 100).toFixed(2)}%` : '-'
                }
                color="#94a3b8"
              />
            </Grid>
            ...
            <Grid size={{ xs: 6, sm: 4, md: 3 }}>
              <MetricCard
                label="盈亏比"
                value={(analysis.profit_loss_ratio ?? 0).toFixed(2)}
                color="#f59e0b"
              />
            </Grid>
          </Grid>
