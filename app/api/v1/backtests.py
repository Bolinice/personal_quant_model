...
def read_backtest_result(backtest_id: int, db: Session = Depends(get_db)):
...
def create_backtest_result_endpoint(backtest_id: int, result: BacktestResultCreate, db: Session = Depends(get_db)):
    return create_backtest_result(backtest_id, result, db=db)

@router.get("/{backtest_id}/trades", response_model=List[BacktestTradeOut])
def read_backtest_trades(backtest_id: int, page: int = 1, page_size: int = 100, db: Session = Depends(get_db)):
