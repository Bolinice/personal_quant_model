...
def read_stock_pool(pool_code: str, db: Session = Depends(SessionLocal)):
...
def create_stock_pool_endpoint(pool: StockPoolCreate, db: Session = Depends(SessionLocal)):
    return create_stock_pool(pool, db=db)

@router.put("/{pool_id}", response_model=StockPoolOut)
def update_stock_pool_endpoint(pool_id: int, pool_update: StockPoolUpdate, db: Session = Depends(SessionLocal)):
...
def create_stock_pool_snapshot_endpoint(pool_id: int, snapshot: StockPoolSnapshotCreate, db: Session = Depends(SessionLocal)):
