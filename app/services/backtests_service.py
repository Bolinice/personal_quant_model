...
def get_stock_price(security_id, current_date, db=None):
    """获取股票价格"""
    stock_data = (
        db.query(StockDaily).filter(StockDaily.ts_code == security_id, StockDaily.trade_date == current_date).first()
    )
...
@with_db
def get_backtests(
    model_id: int | None = None, status: str | None = None, skip: int = 0, limit: int = 100, db: Session | None = None
):
...
def get_backtest_trades(backtest_id: int, page: int = 1, page_size: int = 100, db: Session = None):
    ...
    return (
        db.query(BacktestTrade).filter(BacktestTrade.backtest_id == backtest_id).offset(offset).limit(page_size).all()
    )
...
def initialize_portfolio(model, initial_capital, db):
    """初始化投资组合"""
    # 创建新的模拟组合
    return create_simulated_portfolio(
        {
            "model_id": model.id,
