...
class PerformanceAnalyzer:
    ...
    def calc_downside_volatility(self, returns: pd.Series, target: float = 0, annualize: bool = True) -> float:
        """计算下行波动率"""
    ...
    def calc_information_ratio(self, strategy_returns: pd.Series, benchmark_returns: pd.Series) -> float:
        """计算信息比率"""
    ...
    def calc_treynor_ratio(self, returns: pd.Series, beta: float, risk_free_rate: float = 0.03) -> float:
        """计算特雷诺比率"""
        if beta == 0:
            return 0
    ...
    def calc_beta(self, strategy_returns: pd.Series, benchmark_returns: pd.Series) -> float:
        """计算Beta"""
    ...
    def calc_alpha(
        self, strategy_return: float, benchmark_return: float, beta: float, risk_free_rate: float = 0.03
    ) -> float:
        """计算Alpha"""
    ...
    def calc_industry_attribution(
        self, portfolio_weights: pd.DataFrame, industry_returns: pd.DataFrame
    ) -> pd.DataFrame:
        """
        行业归因分析
        ...
        return pd.DataFrame({"industry": total_contribution.index, "contribution": total_contribution.values})

    def calc_style_attribution(self, portfolio_exposures: pd.DataFrame, style_returns: pd.DataFrame) -> pd.DataFrame:
        """
        风格归因分析
        ...
        return pd.DataFrame({"style": total_contribution.index, "contribution": total_contribution.values})

    # ==================== 完整绩效分析 ====================

    def analyze_performance(
        self, nav_series: pd.Series, benchmark_nav: pd.Series = None, risk_free_rate: float = 0.03
    ) -> dict:
        """
        完整绩效分析
    ...
    def generate_report(
        self, nav_series: pd.Series, benchmark_nav: pd.Series = None, title: str = "绩效分析报告"
    ) -> dict:
        """
        生成完整报告
        ...
        nav_data = pd.DataFrame(
            {"date": nav_series.index, "nav": nav_series.values, "cummax": cummax.values, "drawdown": drawdown.values}
        )
    ...
    def brinson_attribution(
        self,
        portfolio_weights_t: pd.Series,
        portfolio_returns_t: pd.Series,
        benchmark_weights_t: pd.Series,
        benchmark_returns_t: pd.Series,
    ) -> dict:
        """
        Brinson归因分解
        超额收益 = 分配效应 + 选择效应 + 交互效应
        ...
        """
        common = (
            portfolio_weights_t.index.intersection(portfolio_returns_t.index)
    ...
    def factor_return_attribution(
        ...
        specific_returns: pd.Series = None,
    ) -> dict:
        """
        因子收益归因
        ...
        if specific_returns is not None:
            ...
            if len(specific_common) > 0:
                specific_contribution = (
                    portfolio_weights.reindex(specific_common).fillna(0)
                    * specific_returns.reindex(specific_common).fillna(0)
                ).sum()
    ...
    def rolling_performance(
        self,
        returns: pd.Series,
        window: int = 60,  # 为什么60：约一个季度交易日，滚动绩效需足够样本计算统计量
        ...
        # 滚动累计收益
        rolling_cum_return = returns.rolling(window).apply(lambda x: (1 + x).prod() - 1, raw=True)
    ...
    def regime_conditional_performance(self, returns: pd.Series, regime_series: pd.Series) -> dict:
        """
        市场状态条件绩效分析
    ...
    def stress_test_performance(self, returns: pd.Series, stress_periods: list[dict] | None = None) -> dict:
        """
        压力测试绩效分析
        评估策略在A股极端市场环境下的表现
        """
...
@with_db
def analyze_backtest_performance(backtest_id: int, db: Session = None) -> dict | None:
    """
    分析回测绩效
...
@with_db
def analyze_portfolio_performance(portfolio_id: int, db: Session = None) -> dict | None:
    """
    分析组合绩效
    ...
    navs = (
        ...
        .filter(SimulatedPortfolioNav.portfolio_id == portfolio_id)
    ...
    nav_series = pd.Series({n.trade_date: n.nav for n in navs})
