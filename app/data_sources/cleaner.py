...
class CleanReport:
    ...
    def add_issue(self, issue_type: str, message: str, code: str | None = None, trade_date: str | None = None):
        self.issues.append(
            {
                "type": issue_type,
                "ts_code": code,
    ...
    def summary(self) -> str:
        return (
            f"CleanReport: {self.original_count} → {self.cleaned_count} "
...
class DataCleaner:
    ...
    def clean_stock_daily(self, df: pd.DataFrame, code_col: str = "ts_code") -> tuple[pd.DataFrame, CleanReport]:
        """
        清洗股票日线行情
    ...
    def clean_index_daily(self, df: pd.DataFrame, code_col: str = "index_code") -> tuple[pd.DataFrame, CleanReport]:
        """清洗指数日线行情"""
        report = CleanReport(original_count=len(df))
    ...
    def clean_stock_basic(self, df: pd.DataFrame) -> tuple[pd.DataFrame, CleanReport]:
        ...
        if "ts_code" in df.columns:
            ...
            if invalid_count > 0:
                report.add_issue("invalid_ts_code", f"Removed {invalid_count} rows with invalid ts_code format")
                df = df[valid_mask]
        ...
        if "ts_code" in df.columns:
            ...
            if bj_count > 0:
                report.add_issue("bj_filtered", f"Removed {bj_count} Beijing Stock Exchange stocks")
                df = df[~bj_mask]
        ...
        if "industry" in df.columns:
            ...
            if missing > 0:
                ...
                report.add_issue("industry_filled", f"Filled {missing} missing industry values")
        ...
        if "ts_code" in df.columns:
            ...
            dup = before - len(df)
            if dup > 0:
                report.add_issue("duplicate_removed", f"Removed {dup} duplicate ts_code entries")
    ...
    def _remove_missing_critical(
        self, df: pd.DataFrame, critical_cols: list[str], code_col: str | None, report: CleanReport
        ...
        if removed > 0:
            for idx in df[mask].index[:10]:  # 最多记录10条
                ...
                report.add_issue("missing_critical", "Missing critical column value", code=code, trade_date=trade_date)
            if removed > 10:
                report.add_issue("missing_critical", f"... and {removed - 10} more rows with missing critical values")
            df = df[~mask].reset_index(drop=True)

        return df, removed

    def _remove_negative_values(
        self, df: pd.DataFrame, code_col: str | None, report: CleanReport
    ) -> tuple[pd.DataFrame, int]:
        """移除价格/成交量为负的行"""
        price_cols = [c for c in ["open", "high", "low", "close", "pre_close", "volume"] if c in df.columns]
        if not price_cols:
            return df, 0
        ...
        if removed > 0:
            for idx in df[mask].index[:10]:
                ...
                report.add_issue("negative_value", f"Negative values in: {neg_cols}", code=code, trade_date=trade_date)
            df = df[~mask].reset_index(drop=True)

        return df, removed

    def _flag_ohlc_violations(self, df: pd.DataFrame, code_col: str | None, report: CleanReport) -> pd.DataFrame:
        """标记 OHLC 逻辑不一致的行"""
        ...
        if violation.any():
            ...
            report.add_issue("ohlc_violation", f"Flagged {count} rows with OHLC logic violations")

        return df

    def _flag_limit_violations(self, df: pd.DataFrame, code_col: str | None, report: CleanReport) -> pd.DataFrame:
        """标记涨跌停异常（超出正常涨跌幅范围）"""
        ...
        if over_limit.any():
            ...
            report.flagged_count += count
            report.add_issue("limit_violation", f"Flagged {count} rows exceeding daily limit")

        return df

    def _mark_suspended(self, df: pd.DataFrame, code_col: str | None, report: CleanReport) -> pd.DataFrame:
        """标记停牌数据"""
        ...
        if suspended_count > 0:
            report.add_issue("suspended", f"Marked {suspended_count} suspended trading rows")

        return df

    def _fill_non_critical(self, df: pd.DataFrame, code_col: str | None, report: CleanReport) -> pd.DataFrame:
        """非关键列缺失值用前值填充"""
        ...
        if filled > 0:
            report.add_issue("filled_non_critical", f"Forward-filled {filled} non-critical missing values")

        return df

    def _deduplicate(self, df: pd.DataFrame, code_col: str | None, report: CleanReport) -> tuple[pd.DataFrame, int]:
        ...
        if removed > 0:
            report.add_issue("duplicate_removed", f"Removed {removed} duplicate rows")

        return df, removed

    def _deduplicate_by_date(self, df: pd.DataFrame, report: CleanReport) -> tuple[pd.DataFrame, int]:
        ...
        removed = before - len(df)
        if removed > 0:
            report.add_issue("duplicate_removed", f"Removed {removed} duplicate dates")
