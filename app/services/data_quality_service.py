...
def check_price_anomaly(
    ...
    ), {"date": trade_date, "threshold": threshold * 100}).fetchall()
    ...
    return {
        ...
        "anomalies": [
            {"ts_code": row[0], "trade_date": str(row[1]), "pct_chg": row[2]}
...
def check_zero_volume(
    ...
    return {
        ...
        "count": len(zero_vol),
        "records": [
            {"ts_code": row[0], "trade_date": str(row[1]), "close": float(row[2])}
            for row in zero_vol[:20]
        ],
