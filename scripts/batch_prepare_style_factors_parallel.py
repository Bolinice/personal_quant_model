...
def process_single_date(args):
    """处理单个交易日（用于多进程）"""
    trade_date, output_dir, index, total = args

    try:
        # 检查文件是否已存在
        output_file = output_dir / f"{trade_date.strftime('%Y%m%d')}.parquet"
        if output_file.exists():
            return (index, total, trade_date, 'skip', 0, None)

        # 计算风格因子
        factors_df = calculate_style_factors(trade_date)

        if factors_df.empty:
            return (index, total, trade_date, 'empty', 0, None)

        # 保存为Parquet格式
        factors_df.to_parquet(output_file, index=False)

        return (index, total, trade_date, 'success', len(factors_df), None)

    except Exception as e:
        return (index, total, trade_date, 'error', 0, str(e))


def main():
    """批量准备风格因子数据（多核并行）"""
    # 设置日期范围
    start_date = datetime(2025, 4, 23)  # 数据实际起始日期
    end_date = datetime(2026, 4, 17)

    # 获取CPU核心数
    n_cores = cpu_count()
    n_workers = max(1, n_cores - 1)  # 保留1个核心给系统

    print(f"批量准备风格因子数据（多核并行版本）")
    print(f"日期范围: {start_date.date()} 至 {end_date.date()}")
    print(f"CPU核心数: {n_cores}, 使用工作进程: {n_workers}")
    print("=" * 60)
    ...
    # 准备任务参数
    tasks = [(trade_date, output_dir, i+1, len(trade_dates))
    ...
    with Pool(processes=n_workers) as pool:
        for result in pool.imap_unordered(process_single_date, tasks):
            index, total, trade_date, status, stock_count, error = result

            if status == 'success':
                print(f"[{index}/{total}] {trade_date.strftime('%Y-%m-%d')}: 成功 ({stock_count} 只股票)")
                success_count += 1
            elif status == 'skip':
                print(f"[{index}/{total}] {trade_date.strftime('%Y-%m-%d')}: 已存在，跳过")
                skip_count += 1
            elif status == 'empty':
                print(f"[{index}/{total}] {trade_date.strftime('%Y-%m-%d')}: 无数据")
                fail_count += 1
            elif status == 'error':
                print(f"[{index}/{total}] {trade_date.strftime('%Y-%m-%d')}: 失败 - {error}")
                fail_count += 1
