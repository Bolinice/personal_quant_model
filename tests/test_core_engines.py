...
class TestFactorPreprocessor:
    ...
    def test_preprocess_dataframe_with_config(self):
        ...
        result = preprocessor.preprocess_dataframe(df, ["ep_ttm", "ret_1m_reversal"], config=config)
    ...
    def test_preprocess_dataframe_coverage_filter(self):
        ...
        result = preprocessor.preprocess_dataframe(df, ["good_factor", "sparse_factor"], min_coverage=0.6)
...
class TestBarraWLS:
    ...
    def test_wls_vs_ols(self):
        ...
        returns = pd.Series(
            0.5 * exposures["size"] + 0.3 * exposures["value"] + np.random.randn(n) * 0.1, index=range(n)
        )
    ...
    def test_estimate_factor_covariance(self):
        ...
        np.random.seed(42)
        factor_returns = pd.DataFrame(np.random.randn(252, 3) * 0.01, columns=["size", "value", "momentum"])
...
class TestBacktestEngine:
    ...
    def test_limit_check(self):
        ...
        # 主板涨停
        assert engine.is_limit_up(9.99, "main")
        assert not engine.is_limit_up(5.0, "main")

        # 创业板涨停
        assert engine.is_limit_up(19.99, "gem")
        assert not engine.is_limit_up(10.0, "gem")
    ...
    def test_run_backtest_basic(self):
        ...
        def signal_generator(trade_date, universe, state):
            return dict.fromkeys(universe[:2], 0.5)
...
class TestTimingEngine:
    ...
    def test_fuse_signals(self):
        from app.core.timing_engine import FusionMethod, TimingEngine, TimingSignalType
...
class TestPerformanceAnalyzer:
    ...
    def test_rolling_performance(self):
        ...
        np.random.seed(42)
        returns = pd.Series(np.random.randn(252) * 0.01 + 0.0005, index=pd.date_range("2024-01-01", periods=252))
...
class TestTimingSignalEvaluation:
    ...
    def test_evaluate_with_regime(self):
        """测试带市场状态的信号评估"""
        from app.core.timing_engine import MarketRegime, TimingEngine, TimingSignalType
...
class TestT1Restriction:
    ...
    def test_shares_bought_today_reset_at_day_start_not_in_calc_nav(self):
        ...
        engine.calc_nav(state, date(2024, 1, 15), price_data)
    ...
    def test_t1_sell_restriction_enforced_after_buy(self):
        ...
        result = engine.execute_sell(
            state, "600000.SH", 1000, 10.0, date(2024, 1, 15), {"is_suspended": False, "pct_chg": 0, "is_st": False}
        )
    ...
    def test_t1_old_position_sellable_after_buy_more(self):
        ...
        # 卖出800股应成功(原持仓可卖)
        result = engine.execute_sell(
            state, "600000.SH", 800, 10.0, date(2024, 1, 15), {"is_suspended": False, "pct_chg": 0, "is_st": False}
        )
        ...
        result2 = engine.execute_sell(
            state, "600000.SH", 200, 10.0, date(2024, 1, 15), {"is_suspended": False, "pct_chg": 0, "is_st": False}
        )
...
class TestBSELimitPriceFix:
    ...
    def test_bse_limit_pct_is_30(self):
        ...
        """
        from app.core.backtest_engine import NORTH_LIMIT_PCT, ABShareBacktestEngine

        # 常量修复验证
        assert NORTH_LIMIT_PCT == 30.0, "北交所涨跌停幅度应为30%, 不应为20%"

        engine = ABShareBacktestEngine()

        # get_board_type只按.BJ后缀判断北交所
        assert engine.get_board_type("830001.BJ") == "north", ".BJ后缀应判为北交所"
        # 8开头非BJ后缀不应误判为北交所
        assert engine.get_board_type("800001.SZ") == "main", ".SZ后缀的8开头代码应判为主板, 不应误判为北交所"

        # 北交所涨停判断: pct_chg=29.99应为涨停(>=30-0.01)
        assert engine.is_limit_up(29.99, "north"), "北交所29.99%涨幅应为涨停(30%限制)"
        # 北交所20%涨幅不应为涨停(旧bug会把20%当涨停)
        assert not engine.is_limit_up(20.0, "north"), "北交所20%涨幅不应为涨停(30%限制下20%未达涨停)"

        # 获取限价
        assert engine.get_limit_pct("north") == 30.0, "北交所限价百分比应为30"
...
class TestPortfolioOptimizerPhase2:
    ...
    def test_robust_optimize(self):
        ...
        weights = optimizer.robust_mean_variance_optimize(expected_returns, cov, uncertainty, kappa=1.0)
        assert len(weights) == n
    ...
    def test_transaction_cost_aware(self):
        ...
        weights = optimizer.transaction_cost_aware_optimize(expected_returns, cov, prev_weights, linear_cost=0.003)
        assert len(weights) == n
        assert abs(weights.sum() - 1.0) < 0.05
