...
class StockDailyBasic(Base):
    ...
    close = Column(Numeric(20, 4), nullable=True, comment="当日收盘价")
    turnover_rate = Column(Numeric(12, 4), nullable=True, comment="换手率(%)")
    turnover_rate_f = Column(Numeric(12, 4), nullable=True, comment="换手率(基于自由流通股,%)")
    volume_ratio = Column(Numeric(12, 4), nullable=True, comment="量比")
    pe = Column(Numeric(14, 4), nullable=True, comment="市盈率(动态)")
    pe_ttm = Column(Numeric(14, 4), nullable=True, comment="市盈率TTM")
    pb = Column(Numeric(14, 4), nullable=True, comment="市净率")
    ps = Column(Numeric(14, 4), nullable=True, comment="市销率")
    ps_ttm = Column(Numeric(14, 4), nullable=True, comment="市销率TTM")
    dv_ratio = Column(Numeric(12, 4), nullable=True, comment="股息率(%)")
    dv_ttm = Column(Numeric(12, 4), nullable=True, comment="股息率TTM(%)")
    total_mv = Column(Numeric(20, 4), nullable=True, comment="总市值(万元)")
    circ_mv = Column(Numeric(20, 4), nullable=True, comment="流通市值(万元)")
