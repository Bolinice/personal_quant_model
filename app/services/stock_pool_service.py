...
from app.schemas.stock_pools import FilterConfig, StockPoolCreate, StockPoolUpdate
...
def get_stock_pool_snapshot(pool_id: int, trade_date: str, db: Session = None):
    return (
        db.query(StockPoolSnapshot)
        .filter(StockPoolSnapshot.pool_id == pool_id, StockPoolSnapshot.trade_date == trade_date)
        .first()
    )


@with_db
def create_stock_pool_snapshot(
    pool_id: int, trade_date: str, securities: list[str], eligible_count: int, db: Session = None
):
    db_snapshot = StockPoolSnapshot(
        pool_id=pool_id, trade_date=trade_date, securities=securities, eligible_count=eligible_count
    )
    db.add(db_snapshot)
    db.commit()
