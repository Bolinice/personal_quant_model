...
import {
  ...
  TableCell, TableHead, TableRow, Menu, Snackbar, Alert,
} from '@mui/material';
...
export default function ModelList() {
  ...
  const filtered = models.filter((m) => {
    if (search && !m.model_name.toLowerCase().includes(search.toLowerCase()) && !m.model_code.toLowerCase().includes(search.toLowerCase())) return false;
  ...
  const handleMenuClose = () => { setMenuAnchor(null); setMenuModel(null); };
    ...
    <Box>
        ...
        actions={
          <Box sx={{ display: 'flex', gap: 1 }}>
            <Tooltip title={t.models.cardView}>
              <IconButton onClick={() => setViewMode('card')} sx={{ color: viewMode === 'card' ? '#22d3ee' : '#64748b' }}>
                <ViewModuleIcon />
              </IconButton>
            </Tooltip>
            <Tooltip title={t.models.tableView}>
              <IconButton onClick={() => setViewMode('table')} sx={{ color: viewMode === 'table' ? '#22d3ee' : '#64748b' }}>
                <ViewListIcon />
              </IconButton>
            </Tooltip>
            <button
              ...
              style={{
                ...
                color: '#030712', fontWeight: 600, fontSize: '0.875rem',
      ...
      <GlassPanel animate={false} sx={{ mb: 3, p: 2, display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <FormControl size="small" sx={{ minWidth: 120 }}>
          <InputLabel sx={{ color: '#94a3b8', fontSize: '0.875rem' }}>{t.models.availablePools}</InputLabel>
          <Select
            value={poolFilter}
          ...
          >
            ...
            {pools.map((p) => (
              <MenuItem key={p.pool_code} value={p.pool_code}>{p.pool_name}</MenuItem>
            ))}
          </Select>
        </FormControl>
        <FormControl size="small" sx={{ minWidth: 120 }}>
          <InputLabel sx={{ color: '#94a3b8', fontSize: '0.875rem' }}>{t.models.availableFreq}</InputLabel>
          <Select
            value={freqFilter}
      ...
      {viewMode === 'card' && (
        <Grid container spacing={2.5}>
                ...
                >
                  <GlassPanel
                    ...
                    sx={{
                      ...
                      '&:hover': { transform: 'translateY(-2px)', boxShadow: `0 0 24px ${color}15` },
                    }}
                  >
                    {/* Header */}
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1.5 }}>
                      <Box>
                        <Typography sx={{ fontWeight: 700, color: '#e2e8f0', fontSize: '1rem', mb: 0.5 }}>
                          {model.model_name}
                        </Typography>
                        <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap' }}>
                          {pool && <NeonChip label={POOL_NAMES[pool] || pool} size="small" neonColor={neon || 'default'} />}
                          {freq && <NeonChip label={FREQ_LABELS[freq] || freq} size="small" neonColor="indigo" />}
                          <NeonChip label={`v${model.version}`} size="small" neonColor="default" />
                        </Box>
                      </Box>
                      <IconButton size="small" onClick={(e) => handleMenuOpen(e, model)} sx={{ color: '#64748b' }}>
                    ...
                    {/* Description */}
                    {model.description && (
                      <Typography variant="body2" sx={{ color: '#64748b', mb: 1.5, lineHeight: 1.4, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                        {model.description}
                      </Typography>
                    )}
                    ...
                    <Box sx={{ display: 'flex', gap: 2, mb: 1.5 }}>
                      <Box>
                        <Typography variant="caption" sx={{ color: '#64748b' }}>{t.models.annualReturn}</Typography>
                        <Typography sx={{ fontWeight: 600, color: '#22d3ee', fontSize: '0.9rem' }}>
                          {fmt(model.ic_mean, true)}
                        </Typography>
                      </Box>
                      <Box>
                        <Typography variant="caption" sx={{ color: '#64748b' }}>{t.models.sharpe}</Typography>
                    ...
                    {/* Footer */}
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', pt: 1.5, borderTop: '1px solid rgba(148, 163, 184, 0.08)' }}>
          ...
          {filtered.length === 0 && (
            <Grid size={12}>
              <Typography sx={{ textAlign: 'center', color: '#64748b', py: 4 }}>暂无模型数据</Typography>
            </Grid>
          )}
        </Grid>
      )}
          ...
          <GlassTable>
            <Table>
              ...
              <TableBody>
                {filtered.map((model) => {
                  ...
                  return (
                    <TableRow
                      ...
                      sx={{ cursor: 'pointer' }}
                    >
                      <TableCell sx={{ fontWeight: 600, color: '#e2e8f0' }}>{model.model_name}</TableCell>
                      <TableCell sx={{ fontFamily: 'monospace', color: '#94a3b8', fontSize: '0.8rem' }}>{model.model_code}</TableCell>
                      <TableCell>
                        {pool ? <NeonChip label={POOL_NAMES[pool] || pool} size="small" neonColor={POOL_NEON[pool] || 'default'} /> : '-'}
                      </TableCell>
                      <TableCell>{freq ? FREQ_LABELS[freq] || freq : '-'}</TableCell>
                      <TableCell>{model.version}</TableCell>
                      <TableCell>
                        <NeonChip label={model.status === 'active' ? t.models.running : t.models.stopped} size="small" neonColor={model.status === 'active' ? 'green' : 'default'} />
                      </TableCell>
                      <TableCell sx={{ color: '#64748b', fontSize: '0.85rem' }}>{model.updated_at?.slice(0, 10)}</TableCell>
                ...
                {filtered.length === 0 && (
                  <TableRow><TableCell colSpan={8} align="center">暂无模型数据</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </GlassTable>
      ...
      <Menu
        ...
        PaperProps={{ sx: { backgroundColor: 'rgba(15, 23, 42, 0.95)', border: '1px solid rgba(148, 163, 184, 0.15)', backdropFilter: 'blur(16px)' } }}
      >
        <MenuItem onClick={() => { handleMenuClose(); if (menuModel) navigate(`/app/models/${menuModel.model_code}`); }}>
          {t.models.viewDetail}
        </MenuItem>
