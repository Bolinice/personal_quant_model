...
class TestFactorMonitor:
    ...
    def test_ic_drift(self):
        ...
        result = self.monitor.ic_drift(ic_series)
    ...
    def test_psi(self):
        ...
        psi = self.monitor.psi(dist_old, dist_new)
    ...
    def test_psi_identical_distributions(self):
        ...
        psi = self.monitor.psi(dist, dist)
        assert psi < 0.1
    ...
    def test_rolling_ks(self):
        """KS检验应返回统计量和p值"""
        dist_old = _make_factor_dist(mean=0.0)
        dist_new = _make_factor_dist(mean=0.5)
        result = self.monitor.rolling_ks(dist_old, dist_new)
        assert isinstance(result, dict)
